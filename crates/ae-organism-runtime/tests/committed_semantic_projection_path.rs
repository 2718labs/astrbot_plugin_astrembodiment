...
const MATCHING_STATE_AFTER: Digest = [
    175, 101, 127, 47, 72, 127, 104, 128, 15, 110, 181, 235, 113, 84, 247, 80, 133, 31, 121, 0,
    223, 59, 155, 10, 94, 239, 30, 175, 5, 115, 50, 162,
];
const MATCHING_TURN_BINDING: Digest = [
    208, 194, 199, 233, 217, 236, 71, 110, 100, 66, 12, 201, 250, 46, 44, 34, 194, 171, 114, 21,
    12, 235, 142, 178, 178, 117, 213, 119, 64, 99, 42, 108,
];
const MATCHING_SCOPE_DIGEST: Digest = [
    102, 26, 133, 97, 137, 103, 228, 37, 142, 94, 1, 86, 92, 142, 141, 163, 243, 8, 139, 97, 52,
    11, 241, 184, 198, 55, 141, 169, 115, 48, 98, 135,
];
...
fn pre_output_update(
    ...
    let scope_ref = scope_ref_override.unwrap_or_else(|| scope_ref_for(&transition.scope_digest));
...
fn prepared_semantic_transition_commits_only_with_a_matching_pre_output_wire() {
    ...
    let decision = atomically_compile(&mut runtime, constitution.clone(), &user_event([61; 16], 0))
    ...
    assert_eq!(
        wire.wire_digest(),
        &[
            0xa4, 0x37, 0x32, 0x88, 0x15, 0xd2, 0x25, 0x79, 0x8d, 0xb0, 0x51, 0x5d, 0xc3, 0x07,
            0x6c, 0x89, 0x9c, 0x02, 0xb3, 0x07, 0xa3, 0x61, 0x04, 0x72, 0x85, 0xbd, 0xa2, 0xb6,
            0x1d, 0x64, 0xef, 0xd2,
        ],
    ...
    let bytes = wire
        .consume_once()
        .expect("exactly one private consumption");
    assert!(!bytes.is_empty(), "the opaque wire remains consumable once");
    assert!(matches!(
        wire.consume_once(),
...
fn failed_r7_input_leaves_the_same_runtime_retryable_then_good_input_commits() {
    ...
    let rejected_input =
        R7PreOutputProjectionInputV1::new(identity_capsule(constitution.clone()), rejected)
            .expect("identity");
    assert!(matches!(
        runtime.apply_user_stimulus_with_private_projection_wire_v1(&event, &rejected_input),
        Err(RuntimeError::PrivateProjectionWireUnavailable)
    ));
    assert_eq!(runtime.current_revision(), 0);
    assert_eq!(runtime.field.potential, field_before);
...
fn new_r7_commit_does_not_read_or_advance_a_legacy_producer_watermark() {
    let constitution = identity(41);
    let mut runtime = AstrRuntime::scaffold();
    let legacy = NativeProjectionPayloadProducerV1::new(identity_capsule(constitution.clone()))
...
fn migrated_legacy_causal_binding_mismatches_do_not_consume_the_transition() {
    ...
    for (turn_override, scope_override) in [
        ...
        let input =
            R7PreOutputProjectionInputV1::new(identity_capsule(constitution.clone()), update)
                .expect("identity");
