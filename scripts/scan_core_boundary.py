...
RETIRED_MODULES = {"autonomy", "proactive", "proactive_settings", "secret_store", "observatory", "observatory_controls", "display", "ecosystem"}
FORBIDDEN_MEMBERS = {f"astr_embodiment/{name}.py" for name in RETIRED_MODULES}
FORBIDDEN_SETTINGS = {"proactive_enabled", "proactive_frequency", "user_quiet_hours", "proactive_settings_revision", "proactive_daily_max", "min_proactive_cooldown_minutes", "quiet_hours_start", "quiet_hours_end", "quiet_hours_emergency_bypass", "intention_ttl_minutes", "unanswered_backoff_base_minutes", "unanswered_hard_stop", "emergency_threshold", "inner_activity_token_daily_max"}

def manifests(root=ROOT):
    source = (root / "crates/ae-contracts/src/core_surface.rs").read_text(encoding="utf-8")
    def section(name):
    ...
    kinds = re.findall(r'\((\d+),\s*"([^"]+)",\s*(\d+)\)', section("RETIRED_EVENT_KIND_MANIFEST_V1"))
    lp = lambda s: struct.pack("<H", len(s.encode())) + s.encode()
    ...
    ret += struct.pack("<H", len(kinds)) + b"".join(bytes([int(k)]) + lp(n) + bytes([int(r)]) for k,n,r in kinds)
    ...
    if hashlib.sha256(pub).hexdigest() != "800e4dccb2a29b6edbaa2ac7cc46c34f466ece682e0ccf1a272577aacb3790c4" or hashlib.sha256(ret).hexdigest() != "6a28d0e925be34a108b636c47b4ea8ff1a69b763d7de14ba91d4a44127dc291a":
...
def scan_python(source, filename, retired):
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Import) and any(set(a.name.split(".")) & RETIRED_MODULES for a in node.names):
            raise ValueError(f"RETIRED_IMPORT: {filename}")
        if isinstance(node, ast.Constant) and isinstance(node.value, str) and node.value in retired | FORBIDDEN_SETTINGS:
...
def scan_source(root=ROOT):
    ...
    registrations = re.findall(r"wrap_pyfunction!\((\w+),", native)
    ...
    return {"status": "SOURCE_SCAN_PASS", "methods": public, "scope": "active Python AST, PyO3 registration and wrapper exports; Rust execution reachability requires independent review"}
