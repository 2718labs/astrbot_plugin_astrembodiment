#!/usr/bin/env python3
"""Fail closed when the public release version markers disagree."""
...
_METADATA_VERSION = re.compile(r'^version:\s*"([^"]+)"\s*$', re.MULTILINE)
...
class ReleaseContractError(ValueError):
    """A source tree does not describe one coherent release."""
...
def _pep440_version(release_version: str) -> str:
    match = _RELEASE_VERSION.fullmatch(release_version)
    if match is None:
        raise ReleaseContractError(
            f"version mismatch: unsupported metadata version {release_version!r}"
        )
    phase = match.group("phase")
    if phase is None:
        return match.group("release")
    phase_token = {"alpha": "a", "beta": "b", "rc": "rc"}[phase]
    return f"{match.group('release')}{phase_token}{match.group('number')}"
...
def verify_release_contract(root: Path, tag: str | None = None) -> str:
    """Return the expected tag after checking every public version marker."""
    ...
    expected_python = _pep440_version(metadata_version)
    ...
    if python_version != expected_python:
        raise ReleaseContractError(
            f"version mismatch: pyproject.toml={python_version!r}, expected={expected_python!r}"
        )
...
def main() -> int:
    ...
    parser.add_argument("--tag", help="expected Git tag, for example v1.0.0-rc2")
