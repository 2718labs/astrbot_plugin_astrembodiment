...
from astr_embodiment.interaction import persona_scope, build_core_inbound_request
...
@pytest.mark.parametrize("cadence,ceiling", [("active", 300000), ("asleep", 3600000), ("calm", 21600000)])
def test_not_due_is_only_a_poll_and_due_bytes_are_forwarded(cadence, ceiling):
    class Bridge:
        ...
        def embodiment_clock_status_v1(self, request):
            ...
            return {"status": "not_due", "next_due_at_utc_ms": 999999999, "cadence_class": cadence}
        def advance_embodiment_time_v1(self, data):
            assert data is exact
            self.advances.append(data)
            return {"receipt": {"result": {"next_due_at_utc_ms": 999999999, "cadence_class": cadence}}}
    exact = [5, 0, 11, 1, 0, 99]
    locks, bridge = {}, Bridge()
    clock = EmbodimentClock(bridge=bridge, persona_locks=locks, config=lambda k,d:d, now_ms=lambda:1000000)
    clock.prepare_locked = lambda scope: {"profile": {"persona_tzid": "UTC"}, "profile_revision":1,"schedule_revision":1}
    async def run():
...
def test_inventory_epoch_restarts_and_stop_cancels_error_path():
    class Bridge:
        ...
        def list_embodiment_personas_v1(self, request):
            self.calls += 1
            if self.calls == 2: raise RuntimeError("INVENTORY_CHANGED")
            return {"snapshot_token": "11"*32, "entries": [{"scope":SCOPE}],
                    "next_after": SCOPE if self.calls == 1 else None}
        def get_embodiment_persona_v1(self, scope): raise RuntimeError("bounded failure")
    bridge = Bridge()
...
def test_real_native_restart_not_due_and_delivery_after_clock(tmp_path):
    ...
    now = [1773000000000]
    clock = EmbodimentClock(bridge=bridge, persona_locks={}, config=lambda k,d: "UTC" if k=="persona_timezone" else d, now_ms=lambda: now[0])
    event = SimpleNamespace(platform_id="test",bot_id="bot",unified_msg_origin="test:private:one",message_id="message-1")
    def snapshot():
        with sqlite3.connect(f"file:{tmp_path / 'astrembodiment.sqlite3'}?mode=ro", uri=True) as db:
    ...
    try:
        ...
        inbound = build_core_inbound_request(bridge,scope=tokens,event=event,message="hello",appraisal=None,observed_at_ms=now[0])
        ...
        delivery = bridge.compile_core_host_request_v1("delivery",{
            "schema_version":1,"operation_id":"00"*16,"scope":SCOPE,
            "turn_id":observation["turn_id"],"inbound_operation_id":observation["operation_id"],
            "delivered":True,"observed_at_utc_ms":now[0],"visible_action_digest":"11"*32})
        result = bridge.commit_core_delivery_outcome_v1(delivery)
        assert result["commit_status"] == "committed"
        assert bridge.commit_core_delivery_outcome_v1(delivery)["commit_status"] == "existing"
...
def test_real_native_host_semantic_matrix_and_ordinary_delivery(tmp_path):
    ...
    harness.plugin._clock = EmbodimentClock(
        bridge=bridge, persona_locks=harness.plugin._persona_locks,
    ...
    async def run():
        ...
        assert any(item["code"] == "SEMANTIC_COMMITTED" for item in harness.plugin._semantic_diagnostics)
